...
現有專案已經有 `map_enrichment_job.py` 這種由外部排程呼叫的批次入口，因此定期 refresh 最自然的延伸方式，是新增一個 trip refresh job 來協調「列出 active trip -> 逐 trip 重跑 enrichment -> 更新 Mongo canonical data」。這個 change 假設旅次 context 已經存在，並以 active trip 作為唯一刷新範圍。

第一版不追求最細的 pricing-only refresh，而是先用現有 enrichment pipeline 完成一條可運行的定期更新流程。
...
- 逐 trip 執行 lodging/map enrichment 並更新 Mongo canonical data
...
新增一個 coordinator job，先查出 active trips，再逐一呼叫現有 trip-scoped enrichment 邏輯。`map_enrichment_job.py` 保持可單獨執行，trip refresh job 則作為更高一層的協調入口。
...
每次 refresh 直接重跑該 trip 範圍的 enrichment，讓 Mongo-backed 旅次頁與 LINE preview 在下一次讀取時直接看到更新後資料。即使這代表不只價格會更新，仍優先換取較低的實作複雜度。

Rationale:
- 現有系統已經把 enrichment 拆成成熟流程
